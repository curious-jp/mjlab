...
class TerrainViewerOption:
  ...
  supports_parameter_controls: bool = False
  description: str = ""
...
def discover_evaluation_terrain_options() -> dict[str, TerrainViewerOption]:
  """Discover project-local evaluation terrains without hard dependencies."""
  ...
  for terrain_name in _eval_terrain_names(terrains_module):
    options[terrain_name] = TerrainViewerOption(
      ...
      supports_parameter_controls=False,
...
def discover_environment_terrain_options(
  ...
  """Discover registered task environments that use generator terrains."""
...
def main():
  ...
  # State management.
  state: _AppState = {
    "terrain_source": TERRAIN_SOURCE_BUILTIN,
